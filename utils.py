...
def find_exp_num(log_path: str) -> int:
    ...
    if not len(log_files):
        ...
        exp_nums = [os.path.splitext(i)[0].split('/')[-1].split('_')[-1] for i in log_files]
        exp_nums = list(map(int, exp_nums))
        return max(exp_nums) + 1


def remove_abnormal_exp(log_path: str, config_path: str) -> None:
    # logのcsvファイルに1行も記入がないexpの.log, .csv, .yaml を削除する
    log_files = glob(os.path.join(log_path, '*.csv'))
...
def get_logger(config, exp_num):
    ...
    csv_log = mlc.LinewiseCSVWriter(os.path.join(config.log_path, f'exp_{exp_num}.csv'), header=header)
    return logger, csv_log


def save_model(save_name, epoch, loss, acc, model, optimizer, preds, gt):
    state = {
        'epoch': epoch,
        'loss': loss,
        'acc': acc,
...
def save_model_compact(save_name, model):
    state = {
        'weight': model.state_dict()
    }
    torch.save(state, save_name)
